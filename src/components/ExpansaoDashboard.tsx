        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 shadow-xl text-gray-300 font-mono text-sm overflow-hidden flex flex-col h-[600px]">
                <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-800">
                    <Terminal className="w-5 h-5 text-[#C19A5B]" />
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                        <Filter className="w-5 h-5 text-[#C19A5B]" /> Status das Variáveis de Ambiente (Vercel)
                    </h3>
                    <button 
                        onClick={fetchEnv}
                        disabled={loadingEnv}
                ...
                <div className="mt-6 p-4 bg-amber-50 rounded-xl border border-amber-100 flex gap-3 items-start">
                    <div className="bg-amber-100 p-2 rounded-lg text-[#C19A5B]"><Bug className="w-4 h-4" /></div>
            ...
            <div className="flex justify-end">
                <button 
                    ...
                    className="bg-[#C19A5B] hover:bg-amber-600 text-white px-4 py-2 rounded-xl font-bold transition-colors flex items-center gap-2 shadow-sm"
                >
            ...
            {showRedisEditor && (
                <div className="bg-white p-6 rounded-2xl border border-[#E3C78B]/50 shadow-lg">
                    <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                        <Terminal className="w-5 h-5 text-[#C19A5B]" /> Editor do Redis
                    </h3>
                    
                    {loading && Object.keys(redisKeys).length === 0 ? (
                        <div className="flex items-center gap-2 text-[#C19A5B]"><Loader2 className="w-5 h-5 animate-spin"/> Carregando chaves...</div>
                    ) : (
    ...
    >
        <div className="flex justify-between items-start">
            <div className={`p-2.5 rounded-xl ${highlight ? 'bg-white/20 backdrop-blur-sm shadow-inner' : 'bg-amber-50 border border-amber-100/50'}`}>
                <Icon className={`w-5 h-5 ${highlight ? 'text-white' : 'text-[#C19A5B]'}`} />
            </div>
            {trend && (
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between text-sm mb-1.5">
                ...
                <span className="font-bold text-[#C19A5B]">{percentage.toFixed(1)}%</span>
            </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm text-gray-800">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <input 
                                ...
                                className="w-20 bg-white text-center font-bold text-[#C19A5B] outline-none rounded-lg border border-gray-200 focus:border-[#C19A5B] focus:ring-2 focus:ring-amber-100 p-1.5 transition-all"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    ...
                    <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 p-8 rounded-2xl border border-[#E3C78B]/50/50 flex items-center justify-center flex-col text-center relative overflow-hidden shadow-inner">
                        <span className="text-xs text-[#C19A5B] uppercase font-extrabold tracking-widest mb-2">Meta p/ Período Filtrado</span>
                        ...
                        <div className="mt-4">
                            <span className={`text-xs font-bold px-4 py-2 rounded-full shadow-sm ${realizado >= metaPeriodo ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-white text-[#C19A5B] border border-[#E3C78B]/50'}`}>
                ...
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200" onClick={() => setIsVendasModalOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden border border-gray-100" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/80">
                            <h3 className="font-bold text-gray-800 flex items-center gap-2"><Store className="w-5 h-5 text-[#C19A5B]"/> Negociações Ganhas no Período</h3>
                                    ...
                                    {listaVendas.map((venda: any) => (
                                        <div key={venda._id} className="p-4 bg-white border border-gray-200 rounded-xl flex justify-between items-center hover:border-amber-300 transition-colors shadow-sm">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-sm font-extrabold text-[#C19A5B] mt-2">{formatBRL(venda.amount_total || 0)}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
...
const ViewFunil = ({ apiData }: any) => {
    ...
    const stepsSecundario = [
        ...
        { label: 'Agendados', value: 30, color: 'bg-[#E3C78B]' },
        { label: 'Reunião', value: 15, color: 'bg-[#C19A5B]' },
    ...
    const getBarColor = (idx: number, total: number) => {
        if (idx === total - 1) return 'bg-gradient-to-r from-amber-500 to-amber-600 shadow-md shadow-[0_8px_30px_rgb(193,154,91,0.2)]'; 
        if (idx >= total - 3) return 'bg-[#C19A5B]'; 
        if (idx >= total - 6) return 'bg-[#E3C78B]';
        return 'bg-gray-300';
    };
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        <h3 className="text-xl font-bold flex items-center gap-2">
                            <Filter className="w-5 h-5 text-[#C19A5B]" /> Funil Principal: Expansão p9
                        </h3>
                    ...
                    <div className="bg-amber-50 px-5 py-3 rounded-xl border border-amber-100 text-center min-w-[200px] shadow-sm">
                        <p className="text-xs text-[#C19A5B]/80 uppercase font-extrabold tracking-widest mb-1">Eficiência Global</p>
                        <div className="flex flex-col">
                            <span className="text-3xl font-black text-amber-700">{conversaoGeral.toFixed(2)}%</span>
                            <span className="text-[11px] font-medium text-[#C19A5B]/70 mt-1">{totalVendas} vendas / {leadsTotais} leads</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div key={idx} className="relative">
                            ...
                            {idx > 0 && (
                                <div className="absolute top-[-14px] left-[210px] md:left-[210px] z-10">
                                    <div className="flex items-center gap-1 bg-white px-2 py-1 border border-gray-200 rounded-lg shadow-sm text-[10px] font-bold text-[#C19A5B]">
                                        <ArrowDown className="w-3 h-3 text-[#E3C78B]" />
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
        ...
        <div className="bg-white p-10 rounded-2xl border border-amber-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
                ...
                <p className="text-xs text-gray-400 mt-2 font-medium">(Mkt + Agência + Indiretos + Comissões) ÷ <strong className="text-[#C19A5B]">{vendasReais} Vendas</strong></p>
            </div>
            <div className="text-right">
                <h2 className="text-5xl font-black text-[#C19A5B] drop-shadow-sm">{formatBRL(cacReal)}</h2>
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden text-gray-800 group hover:border-[#E3C78B]/50 transition-colors">
                    ...
                    <div className="mt-4 flex items-end gap-2 relative z-10">
                        <span className="text-6xl font-black text-[#C19A5B] drop-shadow-sm">{ratio}x</span>
                    </div>
                ...
                <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden text-gray-800 group hover:border-[#E3C78B]/50 transition-colors">
                    ...
                    <div className="mt-4 flex items-end gap-2 relative z-10">
                        <span className="text-6xl font-black text-[#C19A5B] drop-shadow-sm">{payback}</span>
                ...
                <tbody className="divide-y divide-gray-50">
                    {vendedoresProcessados.length > 0 ? vendedoresProcessados.map((v: any, idx: number) => (
                        <tr key={idx} className="hover:bg-amber-50/30 transition-colors group">
                            ...
                            <td className="p-5 text-center font-black text-[#C19A5B]">
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm overflow-hidden relative">
            <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                <Layout className="w-5 h-5 text-[#C19A5B]"/> Kanban Simplificado (Leads Ativos)
            </h3>
                            ...
                            <div className="flex flex-col gap-3 max-h-[450px] overflow-y-auto pr-1 pb-2 custom-scrollbar">
                                {cards.length > 0 ? cards.map((deal: any) => (
                                    <div 
                                        ...
                                        className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all cursor-pointer hover:border-[#C19A5B] hover:-translate-y-0.5 group"
                                    >
                                        ...
                                        <div className="flex justify-between items-end mt-3">
                                            ...
                                            {deal.value > 0 && <span className="text-xs font-black text-[#C19A5B]">{formatBRL(deal.value)}</span>}
                                        </div>
                                    </div>
                                )) : (
            ...
            <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex justify-end z-50 animate-in fade-in duration-200" onClick={() => setSelectedDeal(null)}>
                ...
                >
                    <div className="p-6 border-b border-gray-100 flex justify-between items-start bg-white sticky top-0 z-10 shadow-sm">
                        <div>
                            <h2 className="text-xl font-black text-[#4A423D] leading-tight">{selectedDeal.name}</h2>
                            <p className="text-xs text-[#C19A5B] font-bold mt-2 uppercase tracking-widest flex items-center gap-1"><UserPlus className="w-3 h-3"/> Responsável: {selectedDeal.user}</p>
                        </div>
                        <button onClick={() => setSelectedDeal(null)} className="p-2 bg-gray-50 rounded-full border border-gray-200 hover:bg-amber-50 hover:text-[#C19A5B] hover:border-[#E3C78B]/50 transition-all">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    ...
                    <div className="p-8">
                        ...
                        {loadingTimeline ? (
                            <div className="flex flex-col items-center gap-4 text-[#C19A5B] justify-center py-12">
                                ...
                                {timelineData.length > 0 ? timelineData.map((item, idx) => (
                                    <div key={idx} className="relative pl-8">
                                        <div className="absolute -left-[15px] top-1 bg-white border-4 border-white rounded-full p-1 shadow-sm ring-1 ring-gray-100">
                                            ...
                                            {item.type === 'fechamento' && <Trophy className="w-4 h-4 text-[#C19A5B]" />}
                                        </div>
                                        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-[#E3C78B]/50 transition-colors group">
                                            ...
                                            <p className="text-sm text-gray-700 leading-relaxed"><strong className="text-[#4A423D]">{fixEncoding(item.title)}:</strong> {fixEncoding(item.desc)}</p>
                                        </div>
                                    </div>
                                )) : (
...
export const ExpansaoDashboard = ({ user, onBack }: any) => {
    ...
    const renderContent = () => {
        if (loading && !apiData) {
            return (
                <div className="flex flex-col justify-center items-center h-[50vh] text-[#C19A5B] gap-6">
                    <Loader2 className="animate-spin w-12 h-12" /> 
                    <div className="text-center w-full max-w-md">
                        <p className="font-bold tracking-widest uppercase text-sm mb-4">{syncState.message}</p>
                        <div className="w-full bg-amber-100 rounded-full h-3 overflow-hidden">
                            <div 
                                className="bg-[#C19A5B] h-3 rounded-full transition-all duration-300 ease-out" 
    ...
    const getBtnStyle = (preset: string) => {
        return activePreset === preset 
            ? "text-[11px] bg-white text-[#C19A5B] font-extrabold px-4 py-2 rounded-xl border-2 border-[#C19A5B] shadow-md shadow-[#C19A5B]/10 whitespace-nowrap transition-all uppercase tracking-wider"
            : "text-[11px] bg-white text-gray-500 hover:text-gray-800 hover:bg-gray-50 font-bold px-4 py-2 rounded-xl border border-gray-200 whitespace-nowrap transition-colors uppercase tracking-wider shadow-sm hover:border-[#E3C78B]/50";
    };
        ...
        <div className="min-h-screen bg-[#FDFBF7] bg-watermark p-4 md:p-8 w-full font-sans selection:bg-[#E3C78B]/30 selection:text-[#4A423D]">
            ...
            {syncState.isSyncing && (
                <div className="fixed top-0 left-0 w-full h-1.5 bg-gray-100 z-50">
                    <div 
                        className="h-full bg-[#C19A5B] transition-all duration-300 ease-out"
                        style={{ width: `${syncState.progress}%` }}
                    ></div>
                </div>
            )}
            ...
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-6">
                <div className="flex items-center gap-4 w-full xl:w-auto">
                    <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-lg shadow-[0_8px_30px_rgb(193,154,91,0.2)] border border-[#C19A5B]/30 overflow-hidden">
                    ...
                    <div>
                        <h1 className="text-2xl font-display font-medium tracking-[0.05em] text-[#4A423D]">EXPANSÃO</h1>
                        <p className="text-[10px] text-[#C19A5B] font-extrabold uppercase tracking-[0.2em]">Painel de Controle Estratégico</p>
                    </div>
                </div>
                        ...
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`flex items-center gap-2 text-sm font-bold transition-all duration-300 ${
                                    activeTab === tab.id 
                                        ? 'bg-[#f59e0b] text-white px-6 py-2.5 rounded-[20px] shadow-md shadow-[0_8px_30px_rgb(193,154,91,0.2)]' 
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-8 flex flex-col md:flex-row gap-6 items-center justify-between">
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="bg-amber-50 p-2 rounded-lg"><Calendar className="w-5 h-5 text-[#C19A5B]" /></div>
                    <input 
                        type="date" 
                        value={startDate} 
                        max={todayStr}
                        onChange={(e) => { setStartDate(e.target.value); setActivePreset(''); }}
                        className="bg-white border border-gray-200 text-gray-700 font-bold text-sm rounded-xl focus:ring-2 focus:ring-amber-400 focus:border-[#C19A5B] block p-2 outline-none shadow-sm transition-all" 
                    />
            ...
            <main className="pb-10">
                <div className="mb-8 flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-black text-[#4A423D] flex items-center gap-3">{activeTab}</h2>
                        <p className="text-sm font-medium text-gray-400 mt-1 uppercase tracking-widest">Métricas e performance em tempo real</p>
                    </div>
                    {loading && <Loader2 className="w-6 h-6 text-[#C19A5B] animate-spin" />}
                </div>
                {renderContent()}
            </main>
            </div>
