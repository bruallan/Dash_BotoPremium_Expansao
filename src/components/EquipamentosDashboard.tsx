...
import { 
    ...
    Package, RefreshCw, CheckCircle2, TrendingUp, TrendingDown, 
...
export function EquipamentosDashboard({ user, onBack }: any) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data: cols, error: errCols } = await supabase.from('colaboradores').select('*');
                if (cols) {
                    setColaboradores(cols.map(c => ({
                        id: c.id,
                        nome: c.nome_colaborador || c.nome, // suporte para "nome colaborador"
                        celular: c.celular,
                        email: c.email,
                        departamento: c.cargo || 'Geral', // mapeando cargo para departamento para não quebrar UI
                        status: c.status
                    })));
                }

                const { data: eqs, error: errEqs } = await supabase.from('equipamentos').select('*');
                if (eqs) {
                    setEquipamentos(eqs.map(e => ({
                        id: e.id,
                        categoriaId: e.categoria_id,
                        modelo: e.modelo,
                        valorCompra: Number(e.valor_compra) || 0,
                        valorReposicao: Number(e.valor_reposicao) || 0,
                        status: e.status || 'Em uso',
                        dataCompra: e.data_compra || new Date().toISOString(),
                        colaboradorId: e.colaborador_id
                    })));
                }

                // Como não foi pedida tabela de manutenções, iniciamos vazio
                setManutencoes([]);
            } catch (e) {
        ...
        <div className="min-h-screen bg-[#F8F9FA] p-4 md:p-8 w-full font-sans selection:bg-amber-100 selection:text-amber-900">
            ...
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-6">
                <div className="flex items-center gap-4 w-full xl:w-auto">
                    ...
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-gray-900 leading-none mb-1">EQUIPAMENTOS</h1>
                        <p className="text-[10px] text-amber-600 font-extrabold uppercase tracking-[0.2em] leading-none">Painel de Controle de Ativos</p>
                    </div>
                </div>
