...
// ENDPOINTS GRANULARES PARA PAGINAÇÃO NO FRONTEND
...
app.post("/api/dashboard/calculate", async (req, res) => {
  ...
  try {
      ...
      (c: any) =>
        ...
        c.categorias.some(
          (cat: any) =>
            (cat.nome || "")
              .toLowerCase()
              .includes("comissões de vendedores") ||
    ...
    console.error("Erro no cálculo do dashboard:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// ENDPOINT ANTIGO (MANTIDO PARA COMPATIBILIDADE SE NECESSÁRIO)
  ...
  try {
    ...
    try {
      ...
      if (e.message.includes("Timeout interno")) {
        debugErrors.push("TIMEOUT: A busca de dados excedeu o limite de segurança de 8.5s.");
      }
    }
